...
def draw_pitch(
    x_min=0, x_max=106, y_min=0, y_max=68,
    pitch_color="w", line_color="grey", line_thickness=1.5, point_size=20,
    orientation="horizontal", aspect="full", ax=None
):
    """
    Draw a football pitch on an existing Matplotlib Axes.
    (Logic aligned with popular 'draw_pitch' helpers; kept as an Axes-based utility.)
    ...
    lx1 = [x_min, x_max, x_max, x_min, x_min]; ly1 = [y_min, y_min, y_max, y_max, y_min]
    lx2 = [x_max, pitch_x[5], pitch_x[5], x_max]; ly2 = [pitch_y[1], pitch_y[1], pitch_y[5], pitch_y[5]]
    lx3 = [0, pitch_x[3], pitch_x[3], 0]; ly3 = [pitch_y[1], pitch_y[1], pitch_y[5], pitch_y[5]]
    lx4 = [x_max, x_max+2, x_max+2, x_max]; ly4 = [goal_y[0], goal_y[0], goal_y[1], goal_y[1]]
    lx5 = [0, -2, -2, 0]; ly5 = [goal_y[0], goal_y[0], goal_y[1], goal_y[1]]
    lx6 = [x_max, pitch_x[7], pitch_x[7], x_max]; ly6 = [pitch_y[2], pitch_y[2], pitch_y[4], pitch_y[4]]
    lx7 = [0, pitch_x[1], pitch_x[1], 0]; ly7 = [pitch_y[2], pitch_y[2], pitch_y[4], pitch_y[4]]
    lx8 = [pitch_x[4], pitch_x[4]]; ly8 = [0, y_max]
...
def create_shot_map_team(
    ...
    """
    Shot map for a team (penalties excluded). Returns (fig, ax).
    Column expectations align with commonly used StatsBomb event schemas
    (e.g., 'type_name', 'shot_outcome_name', 'shot_statsbomb_xg', 'location_x/y', 'team_name').
    """
    ...
    z1 = [1000 * i for i in df_shots["shot_statsbomb_xg"].tolist()]
    z2 = [1000 * i for i in df_goals["shot_statsbomb_xg"].tolist()]
    zo = 12
    ...
    title_str = f"{team_name}'s Shot Map: {total_shots} shots, {total_goals} goals ({total_xg} xG)"
    if _HAS_HIGHLIGHT:
        htext.fig_htext(title_str.replace(team_name, f"<{team_name}>"),
                        0.13, 0.945, highlight_colors=[team_colour],
                        highlight_weights=["bold"], string_weight="bold",
                        fontsize=23, color=text_colour)
    else:
        fig.text(0.13, 0.945, title_str, fontsize=23, fontweight="bold", color=text_colour)

    if subtitle:
        fig.text(0.13, 0.955, subtitle, fontsize=14, color=text_colour)
...
def create_shot_map_player(
    ...
    """
    Shot map for a player (penalties excluded). Returns (fig, ax).
    """
    df = df[df["shot_type_name"] != "Penalty"]
    ...
    title_str = f"{player_name}: {total_shots} shots, {total_goals} goals for {team_of_interest} ({total_xg} xG)"
    if _HAS_HIGHLIGHT:
        t_fmt = title_str.replace(team_of_interest, f"<{team_of_interest}>")
        htext.fig_htext(t_fmt, 0.13, 0.945, highlight_colors=[team_colour],
                        highlight_weights=["bold"], string_weight="bold", fontsize=23, color="black")
    else:
        fig.text(0.13, 0.945, title_str, fontsize=23, fontweight="bold", color="black")

    if subtitle:
        fig.text(0.13, 0.955, subtitle, fontsize=14, color="black")
...
def create_xg_race_chart(
    ...
    """
    xG race chart (minutes < mins_limit). Returns (fig, ax).
    """
    ...
    for i in range(len(df_shots)):
        is_goal = df_shots["shot_outcome_name"][i] == "Goal"
        minute = df_shots["minute"][i]
        if df_shots["team_name"][i] == home_team:
            h_xG.append(df_shots["shot_statsbomb_xg"][i]); h_min.append(minute)
            if is_goal: h_min_goals.append(minute)
        elif df_shots["team_name"][i] == away_team:
            a_xG.append(df_shots["shot_statsbomb_xg"][i]); a_min.append(minute)
    ...
    for s in ["bottom","left"]: ax.spines[s].set_color(text_colour)

    ax.step(x=h_min, y=h_cum, color=home_colour, label=home_team, linewidth=5, where="post")
    ax.step(x=a_min, y=a_cum, color=away_colour, label=away_team, linewidth=5, where="post")
    ax.scatter(x=h_min_goals, y=h_cum_goals, s=1200, color=home_colour, edgecolors=background, marker="*", alpha=1, linewidth=0.5, zorder=2)
    ax.scatter(x=a_min_goals, y=a_cum_goals, s=1200, color=away_colour, edgecolors=background, marker="*", alpha=1, linewidth=0.5, zorder=2)
    ...
    if _HAS_HIGHLIGHT:
        ...
        htext.fig_htext(t_fmt, 0.04, 1.03, highlight_colors=[home_colour, away_colour],
                        highlight_weights=["bold"], string_weight="bold", fontsize=25, color=text_colour)
    else:
        fig.text(0.04, 1.03, title, fontsize=25, fontweight="bold", color=text_colour)

    if subtitle:
        fig.text(0.04, 1.01, subtitle, fontsize=14, color=text_colour)

    ax.set_xlabel("Minute", color=text_colour, fontsize=16)
    ax.set_ylabel("xG", color=text_colour, fontsize=16)
